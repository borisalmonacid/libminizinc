  ...
  public:
    ...
    ret mapArrayLit(const ArrayLit& al) {
      ...
      if (n == 1 && al.min(0) == 1) {
        dl = new DocumentList("[", ", ", "]");
        for (unsigned int i = 0; i < al.size(); i++)
          dl->addDocumentToList(expressionToDocument(al.element(i)));
      } else if (n == 2 && al.min(0) == 1 && al.min(1) == 1) {
        dl = new DocumentList("[| ", " | ", " |]");
        for (int i = 0; i < al.max(0); i++) {
          ...
          for (int j = 0; j < al.max(1); j++) {
            row->
              addDocumentToList(expressionToDocument(al.element(i * al.max(1) + j)));
          }
        ...
        for (unsigned int i = 0; i < al.size(); i++)
          array->addDocumentToList(expressionToDocument(al.element(i)));
