...
namespace MiniZinc {
        ...
        if (base_t.bt() == Type::BT_INT) {
          ...
          if (base_t.st() == Type::ST_PLAIN) {
            for (unsigned int i=0; i<v->size(); i++) {
              IntVal iv = eval_int(env, v->element(i));
              if (!isv->contains(iv)) {
                std::ostringstream oss;
            ...
            for (unsigned int i=0; i<v->size(); i++) {
              IntSetVal* iv = eval_intset(env, v->element(i));
          ...
          if (base_t.st() == Type::ST_PLAIN) {
            for (unsigned int i=0; i<v->size(); i++) {
              FloatVal fv = eval_float(env, v->element(i));
              if (!fsv->contains(fv)) {
                std::ostringstream oss;
            ...
            for (unsigned int i=0; i<v->size(); i++) {
              FloatSetVal* fv = eval_floatset(env, v->element(i));
            ...
            if (vd->e()->type().bt()==Type::BT_INT) {
              ...
              if (vd->e()->type().dim() > 0) {
                ...
                for (unsigned int i=0; i<al->size(); i++) {
                  checkDom(env, vd->id(), isv, al->element(i));
                }
              } else {
  ...
  Expression* eval_arrayaccess(EnvI& env, ArrayLit* al, const std::vector<IntVal>& dims,
    ...
    return al->element(static_cast<unsigned int>(realidx.toInt()));
  }
    ...
    case Expression::E_ARRAYLIT:
      {
        ...
        for (unsigned int i=0; i<al->size(); i++)
          vals[i] = eval_int(env,al->element(i));
        return IntSetVal::a(vals);
      }
      break;
    case Expression::E_COMP:
  ...
  FloatSetVal* eval_floatset(EnvI& env, Expression* e) {
    ...
    switch (e->eid()) {
      ...
      case Expression::E_ARRAYLIT:
      {
        ...
        for (unsigned int i=0; i<al->size(); i++)
          vals[i] = eval_float(env,al->element(i));
        return FloatSetVal::a(vals);
      }
        break;
      case Expression::E_COMP:
          ...
          if (lhs->type().isbool() && rhs->type().isbool()) {
            ...
            try {
              ...
              for (unsigned int i=0; i<al0->size(); i++) {
                if (!Expression::equal(eval_par(env,al0->element(i)), eval_par(env,al1->element(i)))) {
                  return false;
                }
              }
              return true;
  ...
  IntSetVal* eval_boolset(EnvI& env, Expression* e) {
    switch (e->eid()) {
      ...
      case Expression::E_ARRAYLIT:
      {
        ...
        std::vector<IntVal> vals(al->size());
        for (unsigned int i=0; i<al->size(); i++)
          vals[i] = eval_bool(env,al->element(i));
        return IntSetVal::a(vals);
      }
        break;
      case Expression::E_COMP:
    ...
    case Expression::E_ARRAYLIT:
      {
        ...
        for (unsigned int i=al->size(); i--;)
          args[i] = eval_par(env,al->element(i));
    ...
    void vCall(Call& c) {
      if (c.id() == constants().ids.lin_exp || c.id() == constants().ids.sum) {
        ...
        for (unsigned int i=0; i<al->size(); i++) {
          ...
          IntVal cv = le ? eval_int(env,coeff->element(i)) : 1;
          if (cv > 0) {
            if (b.first.isFinite()) {
              if (lb.isFinite()) {
                lb += cv*b.first;
    ...
    void vCall(Call& c) {
      if (c.id() == constants().ids.lin_exp || c.id() == constants().ids.sum) {
        ...
        for (unsigned int i=0; i<al->size(); i++) {
          ...
          FloatVal cv = le ? eval_float(env,coeff->element(i)) : 1.0;
