...
namespace MiniZinc {
  ...
  IntVal b_int_min(EnvI& env, Call* call) {
    ...
    case 1:
      if (args[0]->type().is_set()) {
        ...
        IntVal m = eval_int(env,al->element(0));
        for (unsigned int i=1; i<al->size(); i++)
          m = std::min(m, eval_int(env,al->element(i)));
        return m;
      }
    case 2:
      {
  ...
  IntVal b_int_max(EnvI& env, Call* call) {
    ...
    case 1:
      if (args[0]->type().is_set()) {
        ...
        IntVal m = eval_int(env,al->element(0));
        for (unsigned int i=1; i<al->size(); i++)
          m = std::max(m, eval_int(env,al->element(i)));
        return m;
      }
    case 2:
      {
  ...
  IntVal b_arg_min_int(EnvI& env, Call* call) {
    ...
    int m_idx = 0;
    for (unsigned int i=1; i<al->size(); i++) {
      IntVal mi = eval_int(env,al->element(i));
      if (mi < m) {
        m = mi;
        m_idx = i;
      }
  ...
  IntVal b_arg_min_float(EnvI& env, Call* call) {
    ...
    if (al->size()==0)
      throw ResultUndefinedError(env, al->loc(), "argmin of empty array is undefined");
    FloatVal m = eval_float(env,al->element(0));
    int m_idx = 0;
    for (unsigned int i=1; i<al->size(); i++) {
      FloatVal mi = eval_float(env,al->element(i));
      if (mi < m) {
        m = mi;
        m_idx = i;
      }
  ...
  FloatVal b_array_lb_float(EnvI& env, Call* call) {
    ...
    if (e != NULL) {
      ...
      FloatVal min = 0.0;
      for (unsigned int i=0; i<al->size(); i++) {
        FloatBounds fb = compute_float_bounds(env,al->element(i));
        if (!fb.valid)
    ...
    switch (args.size()) {
      case 1:
        if (args[0]->type().is_set()) {
          ...
          FloatVal m = eval_float(env,al->element(0));
          for (unsigned int i=1; i<al->size(); i++)
            m = std::min(m, eval_float(env,al->element(i)));
          return m;
        }
      case 2:
      {
    ...
    switch (args.size()) {
      case 1:
        if (args[0]->type().is_set()) {
          ...
          FloatVal m = eval_float(env,al->element(0));
          for (unsigned int i=1; i<al->size(); i++)
            m = std::max(m, eval_float(env,al->element(i)));
          return m;
        }
      case 2:
      {
  ...
  IntSetVal* b_array_ub_set(EnvI& env, Call* call) {
    ...
    if (al->size()==0)
      throw EvalError(env, Location(), "upper bound of empty array undefined");
    IntSetVal* ub = b_ub_set(env,al->element(0));
    for (unsigned int i=1; i<al->size(); i++) {
      IntSetRanges isr(ub);
      IntSetRanges r(b_ub_set(env,al->element(i)));
  ...
  IntSetVal* b_dom_array(EnvI& env, Call* call) {
    ...
    if (al->size()==0)
      return IntSetVal::a();
    IntSetVal* isv = b_dom_varint(env,al->element(0));
    for (unsigned int i=1; i<al->size(); i++) {
      IntSetRanges isr(isv);
      IntSetRanges r(b_dom_varint(env,al->element(i)));
      Ranges::Union<IntVal,IntSetRanges,IntSetRanges> u(isr,r);
      isv = IntSetVal::ai(u);
    }
    return isv;
  ...
  Expression* b_array1d_list(EnvI& env, Call* call) {
    ...
    ArrayLit* ret = new ArrayLit(al->loc(), al->_v);
    Type t = al->type();
    t.dim(1);
    ret->type(t);
    ret->flat(al->flat());
  ...
  Expression* b_arrayXd(EnvI& env, Call* call) {
    ...
    ArrayLit* ret = new ArrayLit(al1->loc(), al1->_v, dims);
    Type t = al1->type();
    t.dim(dims.size());
    ret->type(t);
    ret->flat(al1->flat());
  ...
  bool b_xorall_par(EnvI& env, Call* call) {
    ...
    ArrayLit* al = eval_array_lit(env,args[0]);
    for (unsigned int i=al->size(); i--;)
      count += eval_bool(env,al->element(i));
    return count % 2 == 1;
  }
  ...
  bool b_is_fixed_array(EnvI& env, Call* call) {
    ...
    if (al->size()==0)
      return true;
    for (unsigned int i=0; i<al->size(); i++) {
      if (exp_is_fixed(env,al->element(i))==NULL)
        return false;
    }
    return true;
  }
  ...
  Expression* b_fix_array(EnvI& env, Call* call) {
    ...
    for (unsigned int i=0; i<fixed.size(); i++) {
      fixed[i] = exp_is_fixed(env,al->element(i));
      if (fixed[i]==NULL)
        throw EvalError(env, al->element(i)->loc(), "expression is not fixed");
    }
    ...
    if (e->type().isvar()) {
      ...
      if (ArrayLit* al = e->dyn_cast<ArrayLit>()) {
        ...
        for (unsigned int i=0; i<al->size(); i++) {
          ...
          oss << b_show_json_basic(env, al->element(i));
          for (unsigned int j=0; j<dims.size(); j++) {
  ...
  std::string b_concat(EnvI& env, Call* call) {
    ...
    std::ostringstream oss;
    for (unsigned int i=0; i<al->size(); i++) {
      oss << eval_string(env,al->element(i));
    }
    return oss.str();
  }
  ...
  IntSetVal* b_array_union(EnvI& env, Call* call) {
    ...
    if (al->size()==0)
      return IntSetVal::a();
    IntSetVal* isv = eval_intset(env,al->element(0));
    for (unsigned int i=0; i<al->size(); i++) {
      IntSetRanges i0(isv);
      IntSetRanges i1(eval_intset(env,al->element(i)));
  ...
  IntSetVal* b_array_intersect(EnvI& env, Call* call) {
    ...
    if (al->size() > 0) {
      IntSetVal* i0 = eval_intset(env,al->element(0));
      if (i0->size() > 0) {
        IntSetRanges i0r(i0);
        IntVal min = i0r.min();
        while (i0r()) {
        ...
        restart:
          for (int j=al->size(); j--;) {
            IntSetRanges ij(eval_intset(env,al->element(j)));
  ...
  Expression* b_sort_by_int(EnvI& env, Call* call) {
    ...
    for (unsigned int i=0; i<order.size(); i++) {
      a[i] = i;
      order[i] = eval_int(env,order_e->element(i));
    }
    struct Ord {
  ...
  Expression* b_sort_by_float(EnvI& env, Call* call) {
    ...
    for (unsigned int i=0; i<order.size(); i++) {
      a[i] = i;
      order[i] = eval_float(env,order_e->element(i));
    }
    struct Ord {
  ...
  IntVal b_discrete_distribution(EnvI& env, Call* call) {
    ...
    for(unsigned int i = 0; i < al->size(); i++) {
      weights[i] = eval_int(env,al->element(i)).toInt();
    }
#ifdef _MSC_VER
    std::size_t i(0);
