...
#undef MZN_FILL_REIFY_MAP
        ...
        if(has_output_ann) {
          ...
          if (dims) {
            ...
            for (unsigned int i=0; i<dims->size(); i++) {
              IntSetVal* idxset = eval_intset(envi,dims->element(i));
              s << *idxset << ",";
            }
          }
  ...
  EnvI::evalOutput(std::ostream &os) {
    ...
    for (int i=0; i<al->size(); i++) {
      std::string s = eval_string(*this, al->element(i));
      if (!s.empty()) {
        os << s;
        fLastEOL = ( '\n'==s.back() );
      }
  ...
  bool checkDomainConstraints(EnvI& env, Call* c) {
    if (c->id()==constants().ids.int_.le) {
      ...
      if (al_c->size()==1) {
        ...
        IntVal coeff = eval_int(env,al_c->element(0));
        ...
        if (Id* id = al_x->element(0)->dyn_cast<Id>()) {
          if (id->decl()->ti()->domain()) {
            IntSetVal* domain = eval_intset(env,id->decl()->ti()->domain());
        ...
        case Expression::E_ARRAYLIT:
          {
            ...
            if (al->size() > 0 && al->element(0)->type().isint()) {
              IntVal min = IntVal::infinity();
              IntVal max = -IntVal::infinity();
              for (unsigned int i=0; i<al->size(); i++) {
                IntBounds ib = compute_int_bounds(env,al->element(i));
                if (!ib.valid) {
                  min = -IntVal::infinity();
                  max = IntVal::infinity();
                  break;
              ...
              if (vd->e()->type().is_set()) {
                ...
                if (ib.valid) {
                  ...
                  if (call && call->id()==constants().ids.lin_exp) {
                    ...
                    if (al->size()==1) {
                      IntBounds check_zeroone = compute_int_bounds(env, al->element(0));
                      if (check_zeroone.l==0 && check_zeroone.u==1) {
                        ArrayLit* coeffs = eval_array_lit(env, call->args()[0]);
                        std::vector<IntVal> newdom(2);
                        newdom[0] = 0;
                        newdom[1] = eval_int(env, coeffs->element(0))+eval_int(env, call->args()[2]);
          ...
          case Expression::E_CALL:
            {
              ...
              if (c->id() == constants().ids.lin_exp) {
                ...
                std::copy(le_c->_v.begin(),le_c->_v.end(),ncoeff.begin());
      ...
      if (Lit* l = le[i]->dyn_cast<Lit>()) {
        ...
        try {
          ...
          for (unsigned int j=0; j<sc_coeff->size(); j++) {
            coeffv.push_back(sign*LinearTraits<Lit>::eval(env,sc_coeff->element(j)));
            alv.push_back(sc_al->element(j));
          }
        } catch (ArithmeticError& e) {
  ...
  void flatten_linexp_call(EnvI& env, Ctx ctx, Ctx nctx, ASTString& cid, Call* c,
    ...
    if (cid==constants().ids.sum) {
      ...
      for (unsigned int i=coeff->size(); i--;)
        c_coeff[i] = LinearTraits<Lit>::eval(env,coeff->element(i));
    }
    cid = constants().ids.lin_exp;
    std::vector<Val> coeffv;
    std::vector<KeepAlive> alv;
    for (unsigned int i=0; i<al->size(); i++) {
      if (Call* sc = same_call(al->element(i),cid)) {
        if (VarDecl* alvi_decl = follow_id_to_decl(al->element(i))->dyn_cast<VarDecl>()) {
        ...
        for (unsigned int j=0; j<sc_coeff->size(); j++) {
          coeffv.push_back(cd*LinearTraits<Lit>::eval(env,sc_coeff->element(j)));
          alv.push_back(sc_al->element(j));
        }
        d += cd*sc_d;
      } else {
    ...
    for (unsigned int i=alv.size(); i--;) {
      alv_e[i] = alv[i]();
      al_same_as_before = al_same_as_before && Expression::equal(alv_e[i],al->element(i));
    }
    if (al_same_as_before) {
      ...
      if (rd->type().dim()>1) {
        ...
        rd = new ArrayLit(al->loc(),al->_v,dims);
        Type t = al->type();
        t.dim(1);
        rd->type(t);
      }
  ...
  KeepAlive flat_cv_exp(EnvI& env, Ctx ctx, Expression* e) {
    ...
    switch (e->eid()) {
      ...
      case Expression::E_ARRAYLIT:
      {
        ...
        GCLock lock;
        for (unsigned int i=0; i<al->size(); i++) {
          es[i] = flat_cv_exp(env, ctx, al->element(i))();
        }
    ...
    case Expression::E_ARRAYLIT:
      {
        ...
        if (al->flat()) {
          ...
          for (unsigned int i=al->size(); i--;)
            elems_ee[i] = flat_exp(env,ctx,al->element(i),NULL,NULL);
          std::vector<Expression*> elems(elems_ee.size());
          for (unsigned int i=elems.size(); i--;)
            elems[i] = elems_ee[i].r();
          std::vector<std::pair<int,int> > dims(al->dims());
        ...
        if (aa->idx().size()==1 && aa->idx()[0]->isa<ArrayAccess>()) {
          ...
          if (aa_inner->v()->type().ispar()) {
            ...
            for (unsigned int i=0; i<al_inner->size(); i++) {
              GCLock lock;
              IntVal inner_idx = eval_int(env, al_inner->element(i));
              if (inner_idx < al->min(0) || inner_idx > al->max(0))
                goto flatten_arrayaccess;
              composed_e[i] = al->element(inner_idx.toInt()-al->min(0));
            }
    ...
    case Expression::E_CALL:
      {
        ...
        if (ctx.b==C_ROOT && decl->e()==NULL &&
          ...
          nctx.b = C_ROOT;
          for (unsigned int i=0; i<al->size(); i++)
            (void) flat_exp(env,nctx,al->element(i),r,b);
          ret.r = bind(env,ctx,r,constants().lit_true);
        } else {
        ...
        if (it==NULL) {
          ...
          if (v->e()) {
            ...
            if (v->e()->type().dim() > 0) {
              ...
              if (vd->ti()->domain()) {
                for (unsigned int i=0; i<al->size(); i++) {
                  if (Id* ali_id = al->element(i)->dyn_cast<Id>()) {
        ...
        void vVarDeclI(VarDeclI* v) {
          ...
          if (v->e()->type().ispar() && v->e()->type().dim() > 0 && v->e()->ti()->domain()==NULL
            ...
            if (v->e()->type().bt()==Type::BT_INT && v->e()->type().st()==Type::ST_PLAIN) {
              ...
              IntVal ub = -IntVal::infinity();
              for (unsigned int i=0; i<al->size(); i++) {
                IntVal vi = eval_int(env, al->element(i));
                lb = std::min(lb, vi);
                ub = std::max(ub, vi);
              }
              GCLock lock;
              ...
              for (unsigned int i=0; i<al->size(); i++) {
                FloatVal vi = eval_float(env, al->element(i));
                lb = std::min(lb, vi);
                ub = std::max(ub, vi);
              }
              GCLock lock;
            ...
            if (v->e()->e()==NULL) {
              ...
              if (v->e()->type().dim() > 0) {
                ...
                if (v->e()->ti()->domain() != NULL) {
                  ArrayLit* al = eval_array_lit(env,v->e()->e());
                  for (unsigned int i=0; i<al->size(); i++) {
                    if (!checkParDomain(env,al->element(i), v->e()->ti()->domain())) {
              ...
              if (Call* c = vd->e()->dyn_cast<Call>()) {
                ...
                if (c->id() == constants().ids.lin_exp) {
                  if (int_lin_eq) {
                    ...
                    std::copy(le_c->_v.begin(),le_c->_v.end(),nc_c.begin());
